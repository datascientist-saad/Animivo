...
- [ ] Production Vercel deploy of this branch is live at `https://animivo.app`
- [ ] `NEXT_PUBLIC_APP_URL=https://animivo.app`
- [ ] `OPENAI_API_KEY` and `SUPABASE_SERVICE_ROLE_KEY` exist only on Vercel (not in the git repo, not in the Android/iOS project)
- [ ] Supabase redirect URLs include `https://animivo.app/auth/callback` and `animivo://auth/callback`
...
- [ ] App name **Animivo**
- [ ] Associated Domains `applinks:animivo.app`
- [ ] `NEXT_PUBLIC_IOS_TEAM_ID` set; AASA file returns JSON at `/.well-known/apple-app-site-association`
...
- [ ] Review notes include demo login, that the app loads `https://animivo.app`, and how to find legal pages

### Apple review notes (paste)

```
Demo: <email> / <password>
The iOS app is a Capacitor shell that loads the production Animivo site at https://animivo.app.
...
- [ ] Play App Signing SHA-256 in `NEXT_PUBLIC_ANDROID_SHA256_CERTS`; `assetlinks.json` live
...
```
Demo: <email> / <password>
The Android app loads https://animivo.app in a Capacitor WebView.
