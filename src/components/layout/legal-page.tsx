...
export function LegalPage({ title, children }: LegalPageProps) {
  return (
    <div className="min-h-dvh bg-background">
      <header className="mx-auto flex max-w-3xl items-center justify-between px-4 py-6 pt-[max(1.5rem,env(safe-area-inset-top))]">
        <Logo size="sm" />
        <Link href="/" className="inline-flex min-h-11 items-center text-sm text-muted-foreground hover:text-foreground">
          Back
        </Link>
      </header>
      <main id="main-content" className="mx-auto max-w-3xl space-y-6 px-4 pb-[max(4rem,env(safe-area-inset-bottom))]">
